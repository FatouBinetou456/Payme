...
export default function AgencyCard({ agency, onClick }) {
  ...
  return (
    <Link to={`/${agency.serviceName}/${agency.location}`} className="flex-grid">
      <div className="w-full relative drop-shadow-xl h-64 overflow-hidden rounded-xl bg-[#f09df0] justify-center transition-transform transform hover:scale-105" onClick={() => onClick(agency)}>
